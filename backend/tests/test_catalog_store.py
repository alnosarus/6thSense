...
def test_catalog_settings_default_to_the_catalog_and_processed_tiers(monkeypatch):
    ...
    assert settings.bucket == "6thsense-catalog"
    assert settings.prefix == "v2/"
...
def s3_env(monkeypatch):
    ...
    monkeypatch.setenv("CATALOG_S3_BUCKET", "6thsense-catalog")
...
def fake_s3(s3_env, monkeypatch):
    fake = _FakeS3({"v2/catalog.json": (MANIFEST, '"etag-1"')})
...
def test_s3_reads_the_manifest_under_the_version_prefix(fake_s3):
    ...
    assert fake_s3.gets == [("v2/catalog.json", None)]
...
def test_s3_revalidates_with_if_none_match_and_keeps_the_cached_doc(
    ...
    assert fake_s3.gets == [("v2/catalog.json", None), ("v2/catalog.json", '"etag-1"')]
...
def test_s3_keeps_preview_assets_on_the_catalog_tier(fake_s3):
    ...
    assert fake_s3.signed == [
        ("6thsense-catalog", "v2/posters/clip-one.jpg", 900)
    ]
    assert url.startswith("https://6thsense-catalog.s3.")
...
def test_s3_corrupt_manifest_is_an_outage_not_a_crash(s3_env, monkeypatch):
    fake = _FakeS3({"v2/catalog.json": (b"{not json", '"etag-1"')})
    monkeypatch.setattr(S3CatalogStore, "_build_client", lambda self: fake)
...
def test_presigning_uses_the_catalog_key_and_not_the_ambient_one(s3_env, monkeypatch):
    ...
    url = get_store().sign("posters/clip-one.jpg", 900)
    assert url.startswith("https://6thsense-catalog.s3.")
    assert "/v2/posters/clip-one.jpg" in url
